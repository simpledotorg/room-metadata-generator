...
public final class UserRoomDaoWithOverloads_Impl extends User.RoomDao {
  ...
  public UserRoomDaoWithOverloads_Impl(RoomDatabase __db) {
    this.__db = __db;
  ...
  @Override
  public void updateLoggedInStatusForUser(final UUID userUuId,
      final User.LoggedInStatus loggedInStatus) {
    __db.assertNotSuspendingTransaction();
  ...
  @Override
  public Single<Integer> userCount(UUID facilityUuid) {
    final String _sql = "SELECT COUNT(uuid) FROM LoggedInUser";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 0);
    return RxRoom.createSingle(new Callable<Integer>() {
      @Override
  ...
  @Override
  public UserFacilityDetails userAndFacilityDetails(int limit) {
    final String _sql = "\n"
            + "      SELECT U.uuid userId, F.uuid currentFacilityId, F.syncGroup currentSyncGroupId\n"
            + "      FROM LoggedInUser U\n"
            + "      INNER JOIN Facility F ON U.currentFacilityUuid == F.uuid\n"
